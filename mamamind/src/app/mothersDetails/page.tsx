...
//       const mother = await fetchMotherById(id); // Assuming you have a function like this to fetch the mother details
...
//             onClose={handleCloseDetailsModal} // You can remove nextOfKin from here for now
//             nextOfKin={null}  />
// )}
...
"use client";
import React, { useState, useEffect } from 'react';
import { Mother } from '../utils/types'; // Remove NextOfKin if it's not used
import { Search, ChevronLeft, ChevronRight, UserPlus } from 'lucide-react';
import { useMothers } from '../hooks/useMothers';
import MotherRegistrationForm from '../motherRegistrationForm';
import MotherDetailsModal from '../motherDetailModal';
import { fetchMotherById } from '../utils/fetchMotherbyId'; // Ensure this is the correct import
...
const MothersDetails = () => {
  ...
  const handleMotherClick = async (id: number) => {
    try {
      ...
      const mother = await fetchMotherById(id); // Fetch only the mother's details
    ...
    <div className="flex-1 overflow-hidden bg-gray-100 w-full">
      ...
      <main className="mx-auto py-6 sm:px-6 lg:px-8">
        ...
        {isDetailsModalOpen && selectedMother && (
          <MotherDetailsModal
            mother={selectedMother}
            onClose={handleCloseDetailsModal} // Next of kin can be removed for now
            nextOfKin={null}          />
        )}
