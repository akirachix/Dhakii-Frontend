...
//   return (
//     <Layout>
//     <div className="flex-1 overflow-hidden bg-gray-100  ml-64">
...
//               className="w-full pl-10 py-2 border-2 border-teal-500 text-gray-600 rounded-md  focus:outline-none focus:border-teal-600 text-[20px]" 
//               value={searchTerm}
//               onChange={(e) => setSearchTerm(e.target.value)}
//             />
//           </div>
//           <button
//             className="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-orange-600 flex items-center"
      ...
      <div className="flex-1 overflow-hidden bg-gray-100 ml-64 p-4"> {/* Adjusting the layout */}
        <header className="bg-white">
          <div className="mx-auto py-4 px-4 sm:px-6 lg:px-8">
            <h1 className="text-2xl font-bold text-gray-800">Mothers&apos; Details</h1> {/* Reduced font size */}
          </div>
        </header>
        ...
        <main className="mx-auto py-4 sm:px-6 lg:px-8">
          <div className="flex justify-between mb-4">
            {/* Search Input */}
            <div className="relative">
              <Search className="absolute left-3 top-5 text-gray-400" size={16} /> {/* Reduced icon size */}
              <input
                type="text"
                placeholder="Search for a mother"
                className="w-full pl-8 py-1.5 border border-teal-500 text-gray-600 rounded-md text-[14px] focus:outline-none focus:border-teal-600" /* Reduced size */
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            ...
            {/* Add Mother Button */}
            <button
            className="bg-[#F18721] text-white px-4 py-2 rounded-md hover:bg-orange-700 shadow-lg flex items-center text-base" /* More distinct orange */
            onClick={() => setIsRegistrationModalOpen(true)}
            >
            <UserPlus className="mr-2" size={20} />
             Add Mother
              ...
              <thead className="bg-gray-50">
                <tr>
                  {['First Name', 'Last Name', 'Village', 'Marital Status'].map((header) => (
                    <th
                      key={header}
                      className="px-6 py-3 text-left text-xl font-bold text-gray-700 tracking-wider" /* Further reduced font size */
                    >
                      {header}
                    </th>
                  ))}
                ...
                {mothersLoading ? (
                  ...
                  currentMothers.map((mother) => (
                    <tr 
                      ...
                      className="cursor-pointer hover:bg-gray-100"
                    >
                      <td className="px-6 py-4 whitespace-nowrap text-base">{mother.first_name}</td> {/* Reduced font size */}
                      <td className="px-6 py-4 whitespace-nowrap text-base">{mother.last_name}</td> {/* Reduced font size */}
                      <td className="px-6 py-4 whitespace-nowrap text-base">{mother.village}</td> {/* Reduced font size */}
                      <td className="px-6 py-4 whitespace-nowrap text-base">{mother.marital_status}</td> {/* Reduced font size */}
                    </tr>
                  ))
                )}
              </tbody>
            ...
            <div className="flex items-center justify-end mt-4">
              <nav className="flex items-center space-x-2">
                <button
                  ...
                  className="p-2 rounded-full text-teal-600 hover:bg-teal-100 disabled:opacity-50"
                >
                  <ChevronLeft className="h-5 w-4" /> {/* Reduced icon size */}
                </button>
